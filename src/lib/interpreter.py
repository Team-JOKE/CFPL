...
class Interpreter(NodeVisitor):
    ...
    def visit_VariableDeclarationBlock(self, var_decl_node: VariableDeclarationBlock):
        for declaration in var_decl_node.declarations:
            ...
            if isinstance(declaration,VariableDeclaration):
                self.VARIABLES[declaration.value][0] = TokenType(var_decl_node.type_node.value).name
                ...
                print("DEFAULT VALUE HERE" + str(DEFAULT_VALUES.get(var_decl_node.type_node.value)))
                if self.VARIABLES[declaration.value][1] == None:
                    self.VARIABLES[declaration.value][1] = DEFAULT_VALUES.get(var_decl_node.type_node.value)
    ...
    def visit_Variable(self,var_node:Variable):
        if self.VARIABLES.get(var_node.value) != None:       
            return self.VARIABLES[var_node.value][1]
        else:
            self.raise_error("Undeclared Variable: "+var_node.value)
    
    def visit_VariableDeclaration(self,var_dec_node:VariableDeclaration):
        if self.VARIABLES.get(var_dec_node.value) != None:       
    ...
    def visit_BinOp(self, bin_op_node:BinOp):
        ...
        if left_type != right_type:
            if not (left_type == int and right_type == float or left_type == float and right_type == int ):
                self.raise_error("Could not perform operation "+bin_op_node.op.value +" on " 
                    + left_type_name +" and " + right_type_name +"\n" + str(left)+" " +bin_op_node.op.value +" "+ str(right))
    ...
    def visit_Assign(self, assign_node:Assign):
        if self.VARIABLES.get(assign_node.left.value) == None:
        ...
        if var_type != right_value_type :
            self.raise_error("Assignment Error: could not assign type "+right_value_type+" on variable of type "+var_type
                +"\n"+str(assign_node.left.value) +" = " +str(right_value))
